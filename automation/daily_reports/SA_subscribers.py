...
def run_subscriber_extraction(config_file, log_filename, cust_name):
    ...
    try:
        ...
        report_type = db_config.get("system_alert_report_type")  # ← Report type from config
        
        if not vertica_secret_name:
            raise ValueError("Missing 'credentials_path' in config['vertica']")
        if not report_type:
            raise ValueError("Missing 'system_alert_report_type' in config['vertica']")
        ...
        for row in results:
            ...
            mps = row.get("mps", "")
            # FIXED: Added bc2r_schema calculation (was missing)
            ...
            for email in emails:
                try:
                    logging.info(
                        "Calling SA_endcustomer.py for mps:%s, email:%s, schema:%s, subject:%s, cust:%s",
                        mps, email, extracts_schema, email_subject, cust_name
                    )
                    # FIXED: Added bc2r_schema as 2nd argument (was missing before)
                    subprocess.run(
                        ["python3", "/ebs/scrips/daily_reports/SA_endcustomer.py", 
                         email, bc2r_schema, extracts_schema, email_subject, cust_name],
                        check=True
                    )
                except subprocess.CalledProcessError as e:
                    logging.error("Error running SA_endcustomer.py for (%s, %s): %s", cust_name, email, e)
                    send_failure_email(email_config, f"SA_endcustomer.py failed for {cust_name}, {email}\n{e}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 SA_subscribers.py <cust_name>")
        sys.exit(1)
