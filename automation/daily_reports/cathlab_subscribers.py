"""
Cathlab Subscribers Script - Reads credentials from AWS Secrets Manager
...
def run_subscriber_extraction(config_file, log_filename, cust_name):
    ...
    try:
        ...
        report_type = db_config.get("cathlab_report_type")  # ← Report type from config
        
        if not vertica_secret_name:
            raise ValueError("Missing 'credentials_path' in config['vertica']")
        if not report_type:
            raise ValueError("Missing 'cathlab_report_type' in config['vertica']")
        ...
        for row in results:
            ...
            for email in emails:
                try:
                    logging.info(
                        "Calling cathlab_endcustomer.py for mps:%s, email:%s, schema:%s, subject:%s, cust:%s",
                        mps, email, extracts_schema, email_subject, cust_name
                    )
                    subprocess.run(
                        ["python3", "/ebs/scrips/daily_reports/cathlab_endcustomer.py", 
                         email, bc2r_schema, extracts_schema, email_subject, cust_name, dashboard_url],
                        check=True
                    )
                except subprocess.CalledProcessError as e:
                    logging.error("Error running cathlab_endcustomer.py for (%s, %s): %s", cust_name, email, e)
                    send_failure_email(email_config, f"cathlab_endcustomer.py failed for {cust_name}, {email}\n{e}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 cathlab_subscribers.py <cust_name>")
        sys.exit(1)
