"""
MR Subscribers Script - Reads credentials from AWS Secrets Manager
...
def run_subscriber_extraction(config_file, log_filename, cust_name):
    ...
    try:
        ...
        report_type = db_config.get("mr_report_type")  # ← Report type from config
        
        if not vertica_secret_name:
            raise ValueError("Missing 'credentials_path' in config['vertica']")
        if not report_type:
            raise ValueError("Missing 'mr_report_type' in config['vertica']")
        ...
        for row in results:
            ...
            for email in emails:
                try:
                    ...
                    subprocess.run(
                        ["python3", "/ebs/scrips/daily_reports/mr_endcustomer.py", 
